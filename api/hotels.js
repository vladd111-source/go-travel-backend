...
export default async function hotelsHandler(req, res) {
  try {
    ...
    const hotels = Array.isArray(cacheJson)
      ? cacheJson.map(h => {
          const fallbackPrice = h["3"] || h["10"] || h["50"] || 0;
          const price = h.priceFrom || fallbackPrice;

          return {
            id: h.hotelId || h.id || null,
            name: h.hotelName || h.name || "Без названия",
            city: h.city || h.location?.name || fallbackCity,
            price: Math.floor(price / nights),
            fullPrice: price,
